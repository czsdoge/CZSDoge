...
*Setup instructions for a Gitian build of CZSDoge Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the CZSDoge
...
- [Building CZSDoge Core](#building-czsdoge-core)
...
Clone the git repositories for czsdoge and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/czsdoge/czsdoge.git
git clone https://github.com/czsdoge/gitian.sigs.git
```
...
in the czsdoge repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building CZSDoge Core
----------------

To build CZSDoge Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the czsdoge repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/czsdoge/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/czsdoge/czsdoge
...
For example:
```bash
URL=https://github.com/laanwj/czsdoge.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit czsdoge=${COMMIT} --url czsdoge=${URL} ../czsdoge/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit czsdoge=${COMMIT} --url czsdoge=${URL} ../czsdoge/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit czsdoge=${COMMIT} --url czsdoge=${URL} ../czsdoge/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the czsdoge git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../czsdoge/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/czsdoge/czsdoge-detached-sigs.git

BTCPATH=/some/root/path/czsdoge
SIGPATH=/some/root/path/czsdoge-detached-sigs

./bin/gbuild --url czsdoge=${BTCPATH},signature=${SIGPATH} ../czsdoge/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/czsdoge-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/czsdoge-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/czsdoge-osx-build.assert
```
...
[czsdoge/gitian.sigs](https://github.com/czsdoge/gitian.sigs/) repository, or if that's not possible create a pull
