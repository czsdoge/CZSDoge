...
#if defined(NDEBUG)
# error "BroccoliCoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "BroccoliCoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetBroccoliCoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // BroccoliCoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("broccolicoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // BroccoliCoin: BIP30 has been active since inception
    ...
    // BroccoliCoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetBroccoliCoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime, bool fCheckPOW)
{
    ...
    // BroccoliCoin: Disallow AuxPow blocks before it is activated.
    ...
    // BroccoliCoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // BroccoliCoin: We probably want to disable this
    int nLockTimeFlags = 0;
